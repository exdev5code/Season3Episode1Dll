	...
	case eAH_Title:
		{
			...
			int len = pName->len;
			memcpy(this->TitleName[this->TitleCount], &aRecv[len], len);
			this->TitleCount++;
		}
		break;
	case eAH_Class:
		{
			...
			*/
			...
			int len = pName->len;
			memcpy(this->ClassName[this->ClassCount], &aRecv[len], len);
			this->ClassCount++;
		}
		break;
	case eAH_Inject:
		{
			...
			*/
			...
			int len = pName->len;
			memcpy(this->InjectName[this->InjectCount], &aRecv[len], len);
			this->InjectCount++;
		}
		break;
	case eAH_Process:
		{
			...
			*/
			...
			int len = pName->len;
			memcpy(this->ProcessName[this->ProcessCount], &aRecv[len], len);
			this->ProcessCount++;
		}
		break;
	}
...
void AntiCheatPlus::CG_Cheater(char* szText)
{
	...
	g_Protocol.DataSend((LPBYTE)&pRequest, pRequest.h.size);
}
// ----------------------------------------------------------------------------------------------
